...
from dfapp.main import setup_app
from dfapp.services.database.utils import session_getter
from dfapp.services.deps import get_db_service
from dfapp.services.utils import initialize_services
from dfapp.utils.logger import configure, logger
from dfapp.utils.util import update_settings
...
@app.command()
def run(
    host: str = typer.Option("127.0.0.1", help="Host to bind the server to.", envvar="DFAPP_HOST"),
    workers: int = typer.Option(1, help="Number of worker processes.", envvar="DFAPP_WORKERS"),
    timeout: int = typer.Option(300, help="Worker timeout in seconds."),
    port: int = typer.Option(7860, help="Port to listen on.", envvar="DFAPP_PORT"),
    components_path: Optional[Path] = typer.Option(
        Path(__file__).parent / "components",
        help="Path to the directory containing custom components.",
        envvar="DFAPP_COMPONENTS_PATH",
    ),
    ...
    log_level: str = typer.Option("critical", help="Logging level.", envvar="DFAPP_LOG_LEVEL"),
    log_file: Path = typer.Option("logs/dfapp.log", help="Path to the log file.", envvar="DFAPP_LOG_FILE"),
    cache: Optional[str] = typer.Option(
        envvar="DFAPP_LANGCHAIN_CACHE",
...
def run_on_mac_or_linux(host, port, log_level, options, app):
    webapp_process = Process(target=run_dfapp, args=(host, port, log_level, options, app))
...
def run_on_windows(host, port, log_level, options, app):
    """
    Run the Dataformer App server on Windows.
    """
    print_banner(host, port)
    run_dfapp(host, port, log_level, options, app)
    return None
...
def build_new_version_notice(current_version: str, package_name: str):
    ...
    # when a new version of Dataformer App is available.
    ...
    if version_is_prerelease(current_version):
        # curl -s "https://pypi.org/pypi/dfapp/json" | jq -r '.releases | keys | .[]' | sort -V | tail -n 1
...
def print_banner(host: str, port: int):
    ...
    try:
        from dfapp.version import __version__ as dfapp_version  # type: ignore

        is_pre_release |= is_prerelease(dfapp_version)  # Update pre-release status
        notice = build_version_notice(dfapp_version, "dfapp")
        notice = stylize_text(notice, "dfapp", is_pre_release)
        if notice:
            notices.append(notice)
        package_names.append("dfapp")
        package_name = "Dataformer App"
    except ImportError:
        dfapp_version = None

    # Attempt to handle dfapp-base similarly
    if dfapp_version is None:  # This means dfapp.version was not imported
        try:
            from importlib import metadata

            dfapp_base_version = metadata.version("dfapp-base")
            is_pre_release |= is_prerelease(dfapp_base_version)  # Update pre-release status
            notice = build_version_notice(dfapp_base_version, "dfapp-base")
            notice = stylize_text(notice, "dfapp-base", is_pre_release)
            if notice:
                notices.append(notice)
            package_names.append("dfapp-base")
            package_name = "Dataformer App Base"
    ...
    info_text = "Collaborate, and contribute at our [bold][link=https://github.com/DataformerAI/dataformer-app]GitHub Repo[/link][/bold] :rocket:"
...
def run_dfapp(host, port, log_level, options, app):
    """
    Run Dataformer App server on localhost
    """
    try:
        if platform.system() in ["Windows"]:
            # Run using uvicorn on MacOS and Windows
            ...
            from dfapp.server import Dataformer AppApplication

            Dataformer AppApplication(app, options).run()
    except KeyboardInterrupt:
        pass
...
def superuser(
    ...
    log_level: str = typer.Option("error", help="Logging level.", envvar="DFAPP_LOG_LEVEL"),
):
    """
    Create a superuser.
    """
